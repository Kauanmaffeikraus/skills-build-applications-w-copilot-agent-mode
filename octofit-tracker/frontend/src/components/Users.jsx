...
export default function Users() {
  ...
  const endpointUrl = import.meta.env.VITE_CODESPACE_NAME ? codespaceEndpoint : `http://localhost:8000/api/users/`;
  ...
  return (
    <div>
      ...
      <ul>
        {items.map((u, i) => (
          <li key={u._id || u.id || i}>{u.name || u.username || JSON.stringify(u)}</li>
        ))}
      </ul>
    </div>
  );
